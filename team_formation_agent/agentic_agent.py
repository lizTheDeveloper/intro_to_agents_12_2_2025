...
if __name__ == "__main__":
    ...
    agent = Agent(working_directory="/Users/annhoward/intro_to_agents_12_2_2025/team_formation_agent/workdir")
    agent.run("https://mtgjson.com/getting-started/ - using this api, we want to solve the team formation problem with a mtg deck creator. Create a software system that can produce decks that are playable, competitive, and fun. Select strategies that work together, and write an evolutionary testing algorithm to test the decks. Your role is as chief data scientist and you will be responsible for the design, implementation, and testing of the software system and the evolutionary testing algorithm. Implement the spec from the openspec specs directory. Run, and test it, validate the output is high quality, and write a visualization to help us understand the results, and test the visualzation in the browser. Once the spec is fully implemented, you'll be launched once a day to improve the system, and expand it to more strategies, decks, and formats. Feel free to improve the software over time. Our goal is to produce the best MTG deck creation open source project that we can.")
